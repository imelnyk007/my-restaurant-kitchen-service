...
class CookListViewTest(TestCase):
    def setUp(self):
        ...
        self.cook1 = Cook.objects.create_user(username="cook1", password="password")
        self.cook2 = Cook.objects.create_user(username="cook2", password="password")
        self.cook3 = Cook.objects.create_user(username="testcook", password="password")
    ...
    def test_cook_list_view_with_search(self):
        response = self.client.get(reverse("kitchen:cook-list"), {"username": "testcook"})
        self.assertEqual(response.status_code, 200)
    ...
    def test_cook_detail_view(self):
        response = self.client.get(reverse("kitchen:cook-detail", kwargs={"pk": self.cook1.id}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_cook_delete_view(self):
        response = self.client.get(reverse("kitchen:cook-delete", kwargs={"pk": self.cook1.pk}))
        self.assertTemplateUsed(response, "kitchen/cook_confirm_delete.html")
        response = self.client.post(reverse("kitchen:cook-delete", kwargs={"pk": self.cook1.pk}))
